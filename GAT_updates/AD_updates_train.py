...
from AD_updates_model import MultiTaskGNN # CHANGE BY AD


# hyperparameters, we can try different configurations later too
# lr, hidden dim CHANGED BY AD
lr = 0.001
weight_decay = 5e-4
epochs = 300
hidden_dim = 128
...
def train_step(model, data, optimizer):
    ...
    # sum them up
    loss = data.loss_weights[0]*loss_ad + data.loss_weights[1]*loss_pd + data.loss_weights[2]*loss_ftd + data.loss_weights[3]*loss_als # CHANGED BY AD - add weights
...
def eval_model(model, data, mask):
    ...
    with torch.no_grad():
        ...
        total_loss = data.loss_weights[0]*loss_ad + data.loss_weights[1]*loss_pd + data.loss_weights[2]*loss_ftd + data.loss_weights[3]*loss_als # CHANGED BY AD - add weights

        return total_loss.item()


def hp_tuning(data):
    '''Performs hyperparameter tuning for the GAT by iterating through combinations of learning rate, 
    hidden dimensions, dropout rate, attention heads, and weight decay.
    ...
    for lrate, hd, drate, ah, wd in itertools.product(param_grid['learning_rate'], param_grid['hidden_dim'], # iterate through parameters in the grid
                                               param_grid['dropout_rate'], param_grid['attention_heads'], param_grid['weight_decay']):

        hp_tuner = MultiTaskGNN(in_channels=data.num_node_features, hidden_channels=hd, out_channels=4, dropout=drate, heads=ah) # initialize instance of model class
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        hp_tuner = hp_tuner.to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=wd)

        # get current validation loss
        current_loss = eval_model(hp_tuner, data, data.val_mask)

        # update best validation loss and best parameters
        if current_loss < best_loss:
            best_loss = current_loss
            best_params = {'lr' : lrate, 'hidden_dim' : hd, 'dropout' : drate, 'attn_heads' : ah, 'weight decay' : wd}
